...
export default {
  ...
  data() {
    return {
      ...
      notifiedByApp: ['Không', 'Có'],
      customTag: ['Type 1', 'Type 2'],
      sheetName: 'Sheet 1',
      objs: [
        {
          cusCode: 'KH000',
  ...
  methods: {
    ...
    async uploadFile(file) {
      const data = {
        ...
        traceCall:
          'list-customer.component.ts -> import() -> CustomerInfoController.uploadFile()',
      }
      try {
        const response = await Api.person.createPath(
          'customer/import',
          data,
          null,
        )
